...
# In case you want to remove some samples
#outliers<-"x,y,z"
...
# To remove outliers.
# regions <- strsplit(outliers, ",", fixed=TRUE)[[1]]
# remove_lists <- lapply(regions, function(x) which(annot$FID != x))
# remove_list<-seq(1:length(annot$FID))
# for (a in remove_lists) {
#   remove_list<-intersect(a,remove_list)
# }
# covar<-covar[remove_list,remove_list]
# annot<-annot[remove_list,]
